game.import('extension',function(lib,game,ui,get,ai,_status){return {name:'RE：英雄杀',content:function (config,pack){
  ...
  var style2=document.createElement('style');
    ...
    character:{
       ...
       "GXS_qinqiong":["male","GXS_chen",5,["GXS_fanji","GXS_mengshen"],[]],
...
target.addTempSkill('GXS_zirong_2',{player:'phaseEnd'});
          ...
          audio:"ext:RE：英雄杀:true",
				...
				forced:true,
				filter:function(event,player){
					if(event._notrigger.contains(event.player)&&player.storage.GXS_guimian2) return false;
					return true;
				},
				logTarget:'player',
				content:function(){
					if(!player.storage.GXS_guimian2){
						player.storage.GXS_guimian2=[];
					}
					player.storage.GXS_guimian2.add(trigger.player);
					
					player.addTempSkill('GXS_guimian2');
				},
			},
                ...
                filter:function(event,player){
return event.card.name=='sha'&&player.storage.GXS_guimian2&&player.storage.GXS_guimian2.contains(event.target)&&!event.audioed;
},
                logTarget:"target",
                content:function(){
				...
				mod:{
					cardUsableTarget:function(card,player,target){
						if(card.name=='sha'&&player.storage.GXS_guimian2&&player.storage.GXS_guimian2.contains(target)) return true;
					}
				}
			},
 ...
 filter:function (event,player){
  ...
  player.say(['那美好的仗我已经打过了，当跑的路我已经跑尽了，所信的道我已经守住了。','从此以后，有公义的冠冕为我存留，就是按着公义审判的主到了那日要赐给我的，不但赐给我，也赐给凡爱慕他显现的人。'].randomGet());					
	...
	ai:{
        ...
        group:['GXS_retongfu_1','GXS_retongfu_zbchange'],
        subSkill:{
        'zbchange':{
        filter:function(event,player){
    ...
    content:function (){
        ...
        for(var i=0;i<target.storage.GXS_retongfu_equip.length;i++){
            ...
            if(get.type(card)=='equip'){
                ...
                target.storage.GXS_retongfu_equip.splice(i--,1);
            }
        };
        for (var i of event.list) target.chooseUseTarget(i,true)._triggered = null;
		}
    ...
    ai:{
        order:function (skill,player){
            if(player.countCards('h','tao')<0&&player.countCards('hej')<4&&player.countCards('he',{subtype:'equip1'})>0){
                return 10;
            }
            return 1;
        },
 ...
 trigger:{
    ...
    content:function (){
		...
		'GXS_nvxiang':{
            ...
            filter:function (event,player){
                return event.card.name=='sha'&&player.canCompare(event.player)&&event.player!=player;
            },
            prompt:function (event,player){
            return get.translation(event.target)+'被杀了，是否对'+get.translation(event.player)+'发动【女相】';
            },
            content:function (){
                'step 0'
...
return 1;
            ...
            "GXS_huzhu":{
                ...
                for(var i=0;i<event.targets.length;i++){
                if(event.targets[i].hp==1&&event.targets[i]!=player) return true;
                }
                },
                check:function (event,player){
                return get.attitude(player,event.target)>0;
...
juedou:{charlotte:true},
   ...
   //——————————公共状态(不叠加)——————//
        ...
        'GXS_chujia':'初嫁',
       		...
       		'GXS_retongfu_info':'出牌阶段，你可以选择以下操作中的1项执行：</li><li>与场上除你外的1名角色形成【镜像】状态,目标角色的装备区视为与你的装备区相同,同时该角色原装备无法使用（场上有角色处于【镜像】状态时，不能发动）；</li><li>弃掉1张手牌或装备区的牌，取消【镜像】状态。',       		
       		'GXS_shengnv':'圣女',
        ...
        'GXS_meiguo_info':'你的体力小于5时，你不是【杀】的合法目标',
            ...
            "GXS_mengshen":"门神",
            "GXS_mengshen_info":"回合结束阶段，你可选择一名其他角色，若如此做，直到你的下回合开始，所有角色对该角色使用的【杀】或【决斗】均视为对你使用",
            "GXS_quancai":"全才",
            ...
            "GXS_dyzongheng1":"连横",
            "GXS_dyzongheng2":"合纵",
            "GXS_dyzongheng_info":'出牌阶段，你可以将一张方片手牌当做【铁索连环】使用或重铸。',
            "GXS_feiren":"飞刃",
    ...
    intro:'',
    author:'欧尼斯特·渣诚</br>作者：鬼神易</br>作者：藏海</br>数据整理：欧尼斯特·渣诚、勇敢黄牛不怕反撸</br>特别鸣谢：勇敢黄牛不怕反撸 提供素材以及反馈bug<li>版本号1.0.0.5<li>特别地，傻狗暗杀效果更改为杀的结算过程封仅目标技能',
    diskURL:'',
    forumURL:'',
    version:'1.0.0.5',
